...
public class Picture extends SimplePicture
{
    ...
    public void grayscale()
    {
        for (Pixel[] s : this.getPixels2D())
        {
            for (Pixel t : s)
            {
                int avg = (t.getRed() + t.getGreen() + t.getBlue()) / 3;
                t.setRed( avg );
                t.setGreen( avg );
                t.setBlue( avg );
            }
...
//        }
    ...
    public void mirrorDiagonal()
    {
        ...
        { 
            ...
            { 
              topPixel = pixels[row][col]; 
              botPixel = pixels[col][col + (row - col)];
              topPixel.setColor(botPixel.getColor()); 
            } 
        }
    }
    ...
    public void edgeDetection2( int edgeDist )
    {
        ...
        Pixel[][] morePixels = copy.getPixels2D();
        for ( int col = 0; col < morePixels[0].length; col++ )
        {
            for ( int row = 0; row < morePixels.length - 1; row++ )
            {
                leftPixel = morePixels[row][col];
                rightPixel = morePixels[row + 1][col];
                rightColor = rightPixel.getColor();
                if ( leftPixel.colorDistance( rightColor ) > edgeDist )
                {
